...
D1 = 0
while True:
    print('Informe os números do CPF:')
    CPF = input(':')[0:11]
    print(f'CPF Digitado:{CPF}')
    if not CPF.isnumeric():
        print('Favor digitar apenas números:')
    elif len(CPF) < 11:
        print(f"Você informou {len(CPF)} dígitos o CPF tem 11 dígitos")
    elif len(CPF) == 11:
        print(f"Qtde dígitos validada")
        for num in CPF:
